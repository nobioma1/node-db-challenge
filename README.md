...
1. Explain the difference between `Relational Databases` and `SQL`.
  ...
  ```
    Relational database is a type of database which is structured in tables that allows us to identify and access data in relation to another piece of data in the database,
    While SQL is language used to interface with a relational database.
  ```

2. Why do tables need a `primary key`?

  ```
    A Primary Key is needed to uniquely identify a row of data on a relational database table.
  ```
...
4. What do we need in order to have a _many to many_ relationship between two tables.
  ...
  ```
    A table that contains the primary key of items on both tables identify a particular item.
    Could call the table an Intermediary table
  ```
